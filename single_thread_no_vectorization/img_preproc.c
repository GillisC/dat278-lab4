...
{
    float rec, norm;
    float column_sums[512] = {0.0f};
    for (int y = 0; y < height; y++) 
    {
        for (int x = 0; x < width; x++) 
        {
            column_sums[x] += img[y * width + x]; 
        }
    }

    for (int x = 0; x < width; x++) 
    {
        column_sums[x] /= height;
    }
    ...
    for (int y = 0; y < width; y++) 
    {
        ...
        {
            rec = 1.0 / column_sums[x];
            norm = img[y * width + x] * rec;
            img[y * width + x] = norm + mask1[y * width + x] - mask2[y * width + x];
            mask1[y * width + x] -= norm;
            mask2[y * width + x] += norm;
        }
...
int main(int argc, char **argv)
{
    ...
    for (int run = 0; run < RUNS; run++)
    {
        flush_cache();
        double t0 = now_sec();
        preproc_imp(img, mask1, mask2, IMAGE_H, IMAGE_W); // convolution_imp1
        double t1 = now_sec();
